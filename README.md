...
-  FPS
A very fleshed out first-person controller made in logic nodes with a focus on smooth movement. I greatly recommend tying Armory's logic-rate to the frame rate for playing on high-refresh-rate displays. The node setup is complicated, good luck if you want to modify it. I made some effort in trying to import .blends for levels at runtime for some kind of workshop-support, which is lifted straight from ArmorPaint. Also included is an Addon for Blender (gex) which can export a JSON for your .blend. It contains each objects name, and if it has any of the properties `jump , boost , gx , gy , gz` them and their respective values. This was also planned to be used to extend workshop functionality since I did not want any user to need to install all of Armory to make levels for this game.
