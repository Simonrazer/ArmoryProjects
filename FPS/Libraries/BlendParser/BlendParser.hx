...
class BlendParser {
	...
	public function get(type:String):Array<Handle> {
		...
		trace("Member Length: " + handles.length);
		return handles;
	}
	...
	public static function getTypeIndex(dna:Dna, type:String):Int {
		/*for (i in 0...dna.types.length) {
			trace(type+": "+dna.types[i]);
		}*/
		for (i in 0...dna.types.length) {
			if (type == dna.types[i]) { return i; }
		}
		return -1;
	}
		...
		for (i in 0...ds.fieldNames.length) {
			...
			if (name == baseName(dnaName)) {
				...
				if (typeIndex < 12) {
					...
					case "double": {trace("doubleALERT");return 0;} //blend.readf64();
