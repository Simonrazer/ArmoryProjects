...
{
    float3 _272 = float3(uv + ((-1.0f).xx / smSize), compare);
    float result = shadowMap_1.SampleCmp(_shadowMap_1_sampler, _272.xy, _272.z);
    float3 _281 = float3(uv + (float2(-1.0f, 0.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _281.xy, _281.z);
    float3 _292 = float3(uv + (float2(-1.0f, 1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _292.xy, _292.z);
    float3 _303 = float3(uv + (float2(0.0f, -1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _303.xy, _303.z);
    float3 _311 = float3(uv, compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _311.xy, _311.z);
    float3 _322 = float3(uv + (float2(0.0f, 1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _322.xy, _322.z);
    float3 _333 = float3(uv + (float2(1.0f, -1.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _333.xy, _333.z);
    float3 _344 = float3(uv + (float2(1.0f, 0.0f) / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _344.xy, _344.z);
    float3 _355 = float3(uv + (1.0f.xx / smSize), compare);
    result += shadowMap_1.SampleCmp(_shadowMap_1_sampler, _355.xy, _355.z);
    return result / 9.0f;
}
...
float shadowTestCascade(Texture2D<float4> shadowMap_1, SamplerComparisonState _shadowMap_1_sampler, float3 eye_1, float3 p, float shadowsBias_1)
{
    ...
    int param;
    int param_1;
    float4x4 _450 = getCascadeMat(d, param, param_1);
    int casi = param;
    int casIndex = param_1;
    float4x4 LWVP = _450;
    float4 lPos = mul(float4(p, 1.0f), LWVP);
    float3 _465 = lPos.xyz / lPos.w.xxx;
    lPos = float4(_465.x, _465.y, _465.z, lPos.w);
    ...
    {
        _491 = nextSplit - casData[16][casi - 1];
    }
    float splitSize = _491;
    ...
    if ((splitDist <= 0.1500000059604644775390625f) && (casi != 3))
    {
        ...
        float3 _569 = lPos2.xyz / lPos2.w.xxx;
        lPos2 = float4(_569.x, _569.y, _569.z, lPos2.w);
